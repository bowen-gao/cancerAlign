...
def main():
    ...
    for epoch in range(opt.n_epochs):
        ...
        with torch.no_grad():
            ...
            c2_combine = np.concatenate([gen_gene, c2_data])
            c2_combine = PCA(n_components=32, random_state=2).fit_transform(c2_combine)
            ...
            cluster1 = KMeans(n_clusters=2, random_state=3).fit(dis_mat1)
            cluster2 = KMeans(n_clusters=2, random_state=3).fit(dis_mat2)
            import collections
            ...
            dis_mat_combine = euclidean_distances(c2_combine, c2_combine)
            cluster_combine = KMeans(n_clusters=2, random_state=1).fit(dis_mat_combine)
            labels_c1 = cluster2.labels_[: len(time_list1)]
            labels_c2 = cluster2.labels_[len(time_list1) :]
            A = len(labels_c1 == 0)
            B = len(labels_c1 == 1)
            C = len(labels_c2 == 0)
            D = len(labels_c2 == 1)
            ...
            print("fisher", oddsratio)

            import collections
            import matplotlib.pyplot as plt

            print(collections.Counter(cluster1.labels_[: len(time_list1)]))
            print(collections.Counter(cluster2.labels_[: len(time_list1)]))
            print(collections.Counter(cluster2.labels_[len(time_list1) :]))
            from scipy import stats

            ps = stats.ttest_ind((np.array(gen_gene)).T, (np.array(c2_data)).T, axis=1)[
                1
            ]
            print("ttest", len(np.where(ps < 0.01)[0]))
            print(len(ps), max(ps), np.mean(ps))
            ...
            label1_0s = np.where(label1 == 0)
            label1_1s = np.where(label1 == 1)
            label2 = cluster2.labels_
            label2_0s = np.where(label2 == 0)
            label2_1s = np.where(label2 == 1)
            cluster1_0s = np.array(c1_data)[label1_0s].T
            cluster1_1s = np.array(c1_data)[label1_1s].T
            cluster2_0s = gen_gene[label2_0s].T
            cluster2_1s = gen_gene[label2_1s].T
            from scipy import stats

            print(stats.ttest_ind(cluster1_0s, cluster1_1s, axis=1)[1].shape)
            cluster1_p = stats.ttest_ind(cluster1_0s, cluster1_1s, axis=1)[1]
            cluster2_p = stats.ttest_ind(cluster2_0s, cluster2_1s, axis=1)[1]
            print("aaaa", len(np.where(cluster1_p < 0.05)[0]))
            print("bbbb", len(np.where(cluster2_p < 0.05)[0]))
            # print(np.argsort(cluster1_p)[:10])
            # print(np.argsort(cluster2_p)[:10])
            l1 = []
            l2 = []
            for index in np.argsort(cluster1_p)[:10]:
                if index_2_name[index] in id2realname:
                    l1.append(id2realname[index_2_name[index]])
                else:
                    l1.append(index_2_name[index])
            for index in np.argsort(cluster2_p)[:10]:
                if index_2_name[index] in id2realname:
                    l2.append(id2realname[index_2_name[index]])
                else:
                    l2.append(index_2_name[index])
            print(l1)
            print(l2)
            # print(label1.index(0), label2.index(0))
            """
            if epoch % 10 == 9:
                plt.clf()
                plt.plot(p1)
                ...
                plt.savefig(opt.c1 + "_" + opt.c2 + "_pvalue")

                plt.clf()
                plt.plot(scores)
                plt.savefig(opt.c1 + "_" + opt.c2 + "_scores")
                plt.clf()
                plt.plot(dy)
                plt.savefig(opt.c1 + "_" + opt.c2 + "_dy")
