...
def cluster(c1):
    ...
    for n in range(2, 7):
        ...
        cm = consensus_clustering(
            cur_base_cluster, KMeans(n_clusters=n, random_state=3)
        )
...
for n in range(5, 6):
    ...
    plt.savefig(str(n) + "_newtest.png", dpi=600)
"""
...
plt.xscale("log")
plt.xlabel("Survival association $\it{p}$-value\ncancerAlign", fontsize=16)
plt.ylabel("Pan-Cancer clustering\nSurvival association $\it{p}$-value", fontsize=16)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.tight_layout()
sns.despine()
sns.despine(top=True, right=True, left=False, bottom=False)
plt.savefig("new_test.png", dpi=600)
