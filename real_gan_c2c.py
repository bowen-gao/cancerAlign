...
def main():
    ...
    parser.add_argument("--c2", type=str, default="BLCA", help="cancer type for c2")
    parser.add_argument(
    ...
    KIRC and STAD
    """
    id2gene = np.loadtxt("real_id2gene.txt", dtype=str)
    print(id2gene, len(id2gene))
    df = pd.read_csv("real_data.csv", index_col=None)
    ...
    for index, row in c1_late.iterrows():
        patient = row["id"]
        gene_vec = row["gene_value"]
        gene_vec = gene_vec.split(";")
        gene_vec = [log10(float(i) + 1) for i in gene_vec]
        c1_late_data.append(gene_vec)
    c2_late_data = []
    for index, row in c2_late.iterrows():
        ...
        gene_vec = gene_vec.split(";")
        gene_vec = [log10(float(i) + 1) for i in gene_vec]
        for i, gene in enumerate(id2gene):
            if gene in c2_count_late:
                c2_count_late[gene] += gene_vec[i]
            else:
                c2_count_late[gene] = gene_vec[i]
        c2_late_data.append(gene_vec)
    c1_early_data = []
    for index, row in c1_early.iterrows():
        ...
        for i, gene in enumerate(id2gene):
            if gene in c1_count_early:
                c1_count_early[gene] += gene_vec[i]
            else:
                c1_count_early[gene] = gene_vec[i]
        c1_early_data.append(gene_vec)
    c2_early_data = []
    for index, row in c2_early.iterrows():
        ...
        for i, gene in enumerate(id2gene):
            if gene in c2_count_early:
                c2_count_early[gene] += gene_vec[i]
            else:
                c2_count_early[gene] = gene_vec[i]
        c2_early_data.append(gene_vec)
    num_genes = len(id2gene)
    from numpy.random import shuffle

    print(len(c1_early_data), len(c1_late_data), len(c2_early_data), len(c2_late_data))
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, data in enumerate(gen_gene):
            for i, value in enumerate(data):
                ...
                if gene in c2_count_generated_early:
                    c2_count_generated_early[gene] += data[i]
                else:
                    c2_count_generated_early[gene] = data[i]
        ...
        for (a, b) in sorted_count_c2_early:
            if a in name_2_index:
                ...
                colis[index] = b
        for i, (a, b) in enumerate(sorted_count_c2_early_generated):
            if a in name_2_index:
                index = name_2_index[a]
                colis2[index] = b
                if i < 100:
        ...
        for i, (a, b) in enumerate(sorted_count_c2_late):
            if a in name_2_index:
                ...
                if i < 100:
                    c3 += 1
        for i, (a, b) in enumerate(sorted_count_c1_early):
            if a in name_2_index:
                index = name_2_index[a]
                colis4[index] = b
                if i < 100:
        ...
        print(
            stats.spearmanr(colis, colis2, axis=0), c2,
        )
        print(
            stats.spearmanr(colis, colis3, axis=0), c3,
        )
        print(
            stats.spearmanr(colis, colis4, axis=0), c4,
        )
